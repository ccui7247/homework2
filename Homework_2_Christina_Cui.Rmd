...
```{r message=FALSE}
ten_week_hits <- billboard %>% group_by(performer, song, year) %>% 
  summarize(weeks_on_chart = max(week) - min(week)) %>% filter(weeks_on_chart >= 10)

thirty_charts <- ten_week_hits %>% 
  group_by(performer, song) %>% 
  separate_rows(performer, sep = "( Featuring | featuring )") %>%
  group_by(performer) %>%
  summarize(sum_num = n()) %>% filter(sum_num > 30)

ggplot(thirty_charts) + 
  geom_bar(aes(x = sum_num, y = performer), stat = "identity") + 
  labs(x = "Number of Ten Week Hits",
       y = "Performer Names",
       title = "Artists with Over 30 Ten Week Hits")
```

The bar plot above shows the 19 artists who have had more than 30 songs classified as "ten week hits", in which the songs charted for at least 10 weeks. Songs featuring multiple artists have been counted as well; for example, Drake had multiple songs that he was featured on that were 10 week hits. 
